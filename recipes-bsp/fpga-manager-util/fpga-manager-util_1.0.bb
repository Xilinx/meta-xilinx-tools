...
do_install() {
        install -d ${D}/${base_libdir}/firmware/xilinx/base
        if [ -e "base.dtbo" ]; then
            #install base hdf artifacts
            install -Dm 0644 base.dtbo ${D}/${base_libdir}/firmware/xilinx/base/base.dtbo
            if [ "${SOC_FAMILY}" != "versal" ]; then
                newname=`basename *.bit.bin_base | awk -F '.bit.bin_' '{print $1}'`
                install -Dm 0644 *.bit.bin_base ${D}/${base_libdir}/firmware/xilinx/base/${newname}.bit.bin
            else
                #partial pdi in xsa is not yet supported, will need to modify this part once supported
                echo "TODO"
            fi
        ...
        for hdf in ${HDF_LIST}; do
                install -Dm 0644 ${hdf}.dtbo ${D}/${base_libdir}/firmware/xilinx/${hdf}/${hdf}.dtbo
                if [ "${SOC_FAMILY}" != "versal" ]; then
                    newname=`basename *.bit.bin_${hdf} | awk -F '.bit.bin_' '{print $1}'`
                    install -Dm 0644 *.bit.bin_${hdf} ${D}/${base_libdir}/firmware/xilinx/${hdf}/${newname}.bit.bin
                else
                    #partial pdi in xsa is not yet supported, will need to modify this part once supported
                    echo "TODO"
                fi
